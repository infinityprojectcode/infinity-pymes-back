...
export const saveCustomerStats = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.CustomerStats (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
export const updateCustomerStats = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.CustomerStats SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deleteCustomerStats = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.CustomerStats WHERE id = ?;
        `;
