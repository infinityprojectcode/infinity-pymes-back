...
export const getPurchaseOrderItems = async (req, res) => {
    ...
    const query = `
    SELECT * FROM ${db}.PurchaseOrderItems`;
...
export const savePurchaseOrderItems = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.PurchaseOrderItems (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
// Update table data
export const updatePurchaseOrderItems = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.PurchaseOrderItems SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deletePurchaseOrderItems = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.PurchaseOrderItems WHERE id = ?;
        `;
