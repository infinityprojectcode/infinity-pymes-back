...
export const getPurchaseReceipts = async (req, res) => {
    ...
    const query = `
    SELECT * FROM ${db}.PurchaseReceipts`;
...
export const savePurchaseReceipts = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.PurchaseReceipts (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
// Update table data
export const updatePurchaseReceipts = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.PurchaseReceipts SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deletePurchaseReceipts = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.PurchaseReceipts WHERE id = ?;
        `;
