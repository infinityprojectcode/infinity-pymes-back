...
export const getPurchaseHistory = async (req, res) => {
    ...
    const query = `
    SELECT * FROM ${db}.PurchaseHistory`;
...
export const savePurchaseHistory = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.PurchaseHistory (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
export const updatePurchaseHistory = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.PurchaseHistory SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deletePurchaseHistory = async (req, res) => {
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.PurchaseHistory WHERE id = ?;
        `;
