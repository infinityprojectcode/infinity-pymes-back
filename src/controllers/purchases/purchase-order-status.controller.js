...
export const getPurchaseOrderStatus = async (req, res) => {
    ...
    const query = `
    SELECT * FROM ${db}.PurchaseOrderStatus`;
...
export const savePurchaseOrderStatus = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.PurchaseOrderStatus (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
export const updatePurchaseOrderStatus = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.PurchaseOrderStatus SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deletePurchaseOrderStatus = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.PurchaseOrderStatus WHERE id = ?;
        `;
