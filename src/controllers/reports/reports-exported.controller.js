...
export const getReportsExported = async (req, res) => {
    const conn = await getConnection();
    const db = variablesDB.data_base;
    const query = `
    SELECT * FROM ${db}.ReportsExported`;
...
export const saveReportsExported = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.ReportsExported (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
// Update table data
export const updateReportsExported = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.ReportsExported SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deleteReportsExported = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.ReportsExported WHERE id = ?;
        `;
