...
export const saveCashCounts = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.CashCounts (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
// Update table data
export const updateCashCounts = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.CashCounts SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deleteCashCounts = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.CashCounts WHERE id = ?;
        `;
