...
export const getBillingPayments = async (req, res) => {
    ...
    const query = `
    SELECT * FROM ${db}.BillingPayments`;
...
export const saveBillingPayments = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.BillingPayments (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
// Update table data
export const updateBillingPayments = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.BillingPayments SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deleteBillingPayments = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.BillingPayments WHERE id = ?;
        `;
