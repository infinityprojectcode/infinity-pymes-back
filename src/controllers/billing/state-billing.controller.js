...
export const getStateBilling = async (req, res) => {
    const conn = await getConnection();
    const db = variablesDB.database;
    const query = `
    SELECT * FROM ${db}.StateBilling`;
...
export const saveStateBilling = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.StateBilling (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
// Update table data
export const updateStateBilling = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.StateBilling SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deleteStateBilling = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.StateBilling WHERE id = ?;
        `;
