...
export const getSupplierContacts = async (req, res) => {
    ...
    const query = `
    SELECT * FROM ${db}.SupplierContacts`;
...
export const saveSupplierContacts = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.SupplierContacts (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
// Update table data
export const updateSupplierContacts = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.SupplierContacts SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deleteSupplierContacts = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.SupplierContacts WHERE id = ?;
        `;
