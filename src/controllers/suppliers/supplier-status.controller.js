...
export const saveSupplierStatus = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.SupplierStatus (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
export const updateSupplierStatus = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.SupplierStatus SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deleteSupplierStatus = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.SupplierStatus WHERE id = ?;
        `;
