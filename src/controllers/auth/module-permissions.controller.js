...
export const getModulePermissions = async (req, res) => {
    ...
    const query = `
    SELECT * FROM ${db}.ModulePermissions`;
...
export const saveModulePermissions = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.ModulePermissions (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
export const updateModulePermissions = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.ModulePermissions SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deleteModulePermissions = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.ModulePermissions WHERE id = ?;
        `;
