...
export const savePanelConfig = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.PanelConfig (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
export const updatePanelConfig = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.PanelConfig SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deletePanelConfig = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.PanelConfig WHERE id = ?;
        `;
