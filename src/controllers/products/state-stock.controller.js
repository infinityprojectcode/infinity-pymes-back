...
export const getStateStock = async (req, res) => {
    const conn = await getConnection();
    const db = variablesDB.database;
    const query = `
    SELECT * FROM ${db}.StateStock`;
...
export const saveStateStock = async (req, res) => {
    ...
    const insert = await conn.query(
        `INSERT INTO ${db}.StateStock (column1, column2) VALUES (?, ?)`,
        [column1, column2]
    );
...
// Update table data
export const updateStateStock = async (req, res) => {
    ...
    try {
        ...
        const update = await conn.query(
            `UPDATE ${db}.StateStock SET column1 = ?, column2 = ? WHERE id = ?`,
            [column1, column2, id]
        );
...
export const deleteStateStock = async (req, res) => {
    // From URL
    // const { id } = req.params;
    ...
    try {
        ...
        const deleteQuery = `
            DELETE FROM ${db}.StateStock WHERE id = ?;
        `;
